...
class adt(val productLists: List<Product>): RecyclerView.Adapter<adt.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.rv_product_item, parent, false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        ThaivbDB.getDatabase(holder.itemView.context)?.let {
            db = it
        }
        ...
        holder.itemView.cmdDelete.setOnClickListener {
            GlobalScope.launch {
                ...
                (holder.itemView.context as MainActivity).runOnUiThread {
                    Toast.makeText(holder.itemView.context, "ลบข้อมูลเรียบร้อยแล้ว", Toast.LENGTH_LONG).show()
                }
